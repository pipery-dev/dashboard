...
function toBitbucketPath(pathOrUrl) {
...
async function bitbucketPaged(path, token) {
  ...
  while (nextPath) {
    const payload = await bitbucketRequest(nextPath, token);
...
export async function listBitbucketRepos(token) {
  const workspaces = await bitbucketPaged("/user/workspaces?pagelen=100", token);
  ...
  for (const workspace of workspaces) {
    ...
    const repos = await bitbucketPaged(
