...
function providerCookieNames(provider) {
  const prefixes = new Set([sessionCookiePrefix(), DEFAULT_SESSION_COOKIE_PREFIX, "__Secure-pipery-auth"]);
  return [
...
function matchesLogoutCookieName(name, provider) {
  return name.includes("next-auth") || name.includes("pipery-auth") || name.includes("pipery-dex");
}

export function expirePiperyAuthCookies(response, request, provider) {
  ...
  const namesToExpire = new Set([
    ...
    ...requestCookieNames.filter(
      (name) =>
        ...
        (!provider && matchesLogoutCookieName(name, provider))
    )
  ]);
